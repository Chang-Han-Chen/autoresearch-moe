...
class CausalSelfAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.branch_scale = 1.0 / math.sqrt(layer_idx + 1) if config.branch_layer_scale else 1.0
...
class Block(nn.Module):
    ...
    def forward(self, x, first_v, cos_sin, window_size):
        ...
        if self.moe is not None:
            ffn_out, aux_loss, stats = self.moe(norm(x), expert_input_scale=self.branch_scale)
        else:
            ffn_x = norm(x)
            if self.branch_scale != 1.0:
                ffn_x = ffn_x * self.branch_scale
...
config = GPTConfig(
    ...
    branch_layer_scale=BRANCH_LAYER_SCALE,
...
if IS_MASTER:
    ...
    print(f"branch_layer_scale: {int(BRANCH_LAYER_SCALE)}")
